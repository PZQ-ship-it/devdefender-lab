...
from devdefender_lab.media_router import contains_forbidden_media_artifact_fields  # noqa: E402
...
DEFAULT_REPORT_PATH = ROOT / "artifacts" / "webrtc_meeting_smoke.json"
WEBRTC_SOURCE = "generic-webrtc-test"


def main() -> int:
    parser = argparse.ArgumentParser(description="Verify Phase 3C generic WebRTC meeting adapter.")
    ...
    parser.add_argument("--timeout", type=float, default=30.0, help="Seconds to wait for WebRTC timeline events.")
...
def run_smoke(room_url: str, out: Path, browser: str | None = None, timeout: float = 30.0) -> dict[str, object]:
    ...
    _post_json(
        f"{room_url}/api/timeline-event",
        {
            ...
            "command": "goto",
            "slide_index": 1,
            "source": "webrtc-meeting-smoke",
        },
    )
    before = _get_json(f"{room_url}/api/timeline-events")
    before_events = _events(before)
    target = local_webrtc_meeting_test_url(room_url)

    profile_dir_obj = tempfile.TemporaryDirectory(prefix="devdefender-webrtc-profile-", ignore_cleanup_errors=True)
    ...
    used_kill = False
    try:
        timeline = wait_for_webrtc_events(room_url, len(before_events), timeout)
    finally:
...
def build_report(
    *,
    ...
    media_error = _last_event(new_events, "media_route_error")
    source_ok = all(_event_source(event) == WEBRTC_SOURCE for event in new_events if _is_webrtc_event(event))
    ...
    checks = {
        ...
        "no_media_route_error": media_error is None,
        "webrtc_source_used": source_ok,
        ...
        and not contains_forbidden_media_artifact_fields(new_events),
    ...
    return {
        ...
        "last_media_route_error": media_error,
    }


def wait_for_webrtc_events(room_url: str, baseline_count: int, timeout: float) -> dict[str, object]:
    ...
    while time.monotonic() < deadline:
        ...
        if (
            _last_event(new_events, "meeting_joined")
            and _last_event(new_events, "media_published")
            and _last_event(new_events, "meeting_left")
        ):
...
def local_webrtc_meeting_test_url(room_url: str) -> str:
    parsed = urllib.parse.urlparse(room_url)
    query = urllib.parse.parse_qsl(parsed.query, keep_blank_values=True)
    query.append(("auto_webrtc_meeting", "1"))
    query.append(("meeting_url", "https://webrtc.local/devdefender?token=redacted-test-token"))
    return urllib.parse.urlunparse(parsed._replace(path="/webrtc-meeting-test", query=urllib.parse.urlencode(query)))
...
def _is_webrtc_event(event: dict[str, object]) -> bool:
