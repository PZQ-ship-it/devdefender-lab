...
def main() -> None:
    parser = argparse.ArgumentParser(description="Verify TTS anchor words drive slide sync through timeline and WebSocket replay.")
    parser.add_argument("--room-url", default=DEFAULT_ROOM_URL, help="Running DevDefender room URL.")
    parser.add_argument("--token", default="next", help="TTS anchor token to post.")
    ...
    with open_slide_socket(room_url) as ws:
        ...
        posted = _post_json(
            ...
            {"kind": "tts_word", "token": args.token, "source": "tts-anchor-smoke", "offset_ms": 1250},
        )
    ...
    report = build_report(snapshot, posted, broadcast, replay, baseline_timeline, timeline)
...
def build_report(
    ...
    timeline_event = posted.get("timeline_event") if isinstance(posted.get("timeline_event"), dict) else {}
    slide_event = posted.get("slide_event") if isinstance(posted.get("slide_event"), dict) else {}
    ...
    baseline_events = baseline_timeline.get("events") if isinstance(baseline_timeline.get("events"), list) else []
    new_timeline_events = timeline_events[len(baseline_events) :]
    last_new_timeline = new_timeline_events[-1] if new_timeline_events and isinstance(new_timeline_events[-1], dict) else {}
    ...
    checks = {
        ...
        "timeline_event_is_tts_word": timeline_event.get("kind") == "tts_word",
        "timeline_source_matches": timeline_event.get("source") == "tts-anchor-smoke",
        "slide_event_next": slide_event.get("action") == "next",
        "slide_event_advances_snapshot": slide_event.get("slide_index") == expected_next,
        "broadcast_matches_slide_event": _event_identity(broadcast) == _event_identity(slide_event),
        ...
        "timeline_replay_contains_new_anchor": _timeline_identity(last_new_timeline)
        == _timeline_identity(timeline_event),
    }
    return {
        ...
        "new_timeline_event_count": len(new_timeline_events),
...
def _timeline_identity(event: dict[str, object]) -> dict[str, object]:
    return {
        "kind": event.get("kind"),
        "source": event.get("source"),
...
def open_slide_socket(room_url: str) -> socket.socket:
    ...
    if parsed.scheme == "https":
        raise ValueError("tts_anchor_smoke only supports local http rooms.")
