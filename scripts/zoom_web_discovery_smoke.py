...
from devdefender_lab.meeting import contains_forbidden_meeting_artifact_fields, redact_meeting_url  # noqa: E402
...
DEFAULT_REPORT_PATH = ROOT / "artifacts" / "zoom_web_discovery_smoke.json"
DEFAULT_ZOOM_URL = "https://zoom.us/wc/join/123456789?pwd=redacted-test-token"
ZOOM_DISCOVERY_SOURCE = "zoom-web-discovery"


def main() -> int:
    parser = argparse.ArgumentParser(description="Verify Phase 3C Zoom Web adapter discovery on native browser automation.")
    ...
    parser.add_argument("--zoom-url", default=DEFAULT_ZOOM_URL, help="Zoom Web URL used only as redaction input to the local fixture.")
    parser.add_argument("--timeout", type=float, default=20.0, help="Seconds to wait for discovery timeline events.")
    ...
    try:
        ...
        report = run_smoke(args.room_url.rstrip("/"), args.out, args.browser, args.timeout, args.zoom_url)
...
def run_smoke(
    ...
    _post_json(
        f"{room_url}/api/timeline-event",
        {
            ...
            "command": "goto",
            "slide_index": 1,
            "source": "zoom-web-discovery-smoke",
        },
    )
    before = _get_json(f"{room_url}/api/timeline-events")
    before_events = _events(before)
    target = local_zoom_discovery_test_url(room_url, zoom_url)

    profile_dir_obj = tempfile.TemporaryDirectory(prefix="devdefender-zoom-discovery-profile-", ignore_cleanup_errors=True)
    ...
    used_kill = False
    try:
        timeline = wait_for_zoom_discovery_events(room_url, len(before_events), timeout)
    finally:
...
def build_report(
    *,
    ...
    source_ok = all(_event_source(event) == ZOOM_DISCOVERY_SOURCE for event in new_events if _is_zoom_discovery_event(event))
    redacted_target_url = redact_meeting_url(target_url) or ""
    join_command = str(join_started.get("command", "")) if join_started else ""
    ...
    checks = {
        ...
        "zoom_prejoin_detected": joined is not None and joined.get("command") == "zoom-prejoin-detected",
        "meeting_left_recorded": left is not None,
        "no_meeting_error": error is None,
        "zoom_discovery_source_used": source_ok,
        "target_url_redacted": _has_no_raw_zoom_secret(redacted_target_url),
        "join_command_redacted": _has_no_raw_zoom_secret(join_command),
    ...
    return {
        ...
        "detected_controls": ["display_name", "join_button", "audio_toggle", "video_toggle", "leave_button"],
    }


def wait_for_zoom_discovery_events(room_url: str, baseline_count: int, timeout: float) -> dict[str, object]:
...
def local_zoom_discovery_test_url(room_url: str, zoom_url: str = DEFAULT_ZOOM_URL) -> str:
    parsed = urllib.parse.urlparse(room_url)
    query = urllib.parse.parse_qsl(parsed.query, keep_blank_values=True)
    query.append(("auto_zoom_discovery", "1"))
    query.append(("zoom_url", zoom_url))
    return urllib.parse.urlunparse(parsed._replace(path="/zoom-discovery-test", query=urllib.parse.urlencode(query)))
...
def _is_zoom_discovery_event(event: dict[str, object]) -> bool:
...
def _has_no_raw_zoom_secret(value: str) -> bool:
    lowered = value.lower()
    forbidden_fragments = {
        "123456789",
        "redacted-test-token",
