...
def test_mock_briefing_adapter_returns_stakeholder_report() -> None:
    ...
    assert report.evidence_pointers[0].pointer == "timeline://thread-1#event=0&kind=briefing_generated"
...
def test_default_briefing_context_builds_serializable_report() -> None:
    ...
    assert payload["evidence_pointers"] == [
        {
            "pointer": "timeline://briefing#event=0&kind=briefing_generated",
            "label": "Primary briefing timeline pointer",
        },
        {
            "pointer": "slide://briefing#page=1",
...
def test_forbidden_briefing_artifact_field_detector_checks_nested_payloads() -> None:
    assert contains_forbidden_briefing_artifact_fields({"safe": ["timeline://thread#event=0&kind=briefing_generated"]}) is False
